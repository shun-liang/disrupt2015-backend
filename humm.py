...
def find_artist_id(authorzation_token,artist):
    string="https://humm-api.p.mashape.com/artists?auth="+authorzation_token+"&keyword="+artist+'&limit=4'
...
def find_top_songs(authorzation_token,artist_id):
    string='https://humm-api.p.mashape.com/artists/'+artist_id+'/topsongs?auth='+authorzation_token+'&limit=4'
    ...
    if response.code != 200:
        ...
        print 'len(data_response): %s' % len(response.body['data_response'])
